...
La presente  unidad será desarrollada durante la comprendida entre 16  y el 22 de mayo.    
Ademas del material suministrado  contaran con el acompañamiento del profesor en tres sesiones (Lunes, Miércoles y Viernes) y de manera asincrónica con  foro de actividades académicas. Los entegables para esta unidad podrán enviarse a través de la plataforma Blackboard hasta el  22 de mayo.
...
|:--------------|:-----------------------------  |
|Actividad 511  | Resuelva los problemas presentados en el [**Caso C&A**](https://dgonxalex80.github.io/probabilidad20212.io/casoCyA.html) . La solución deberá ser entregada mediante formato pdf en link abierto para tal fin en Brightspace 
|Trabajo individual |                             |
...
```{r b1, echo=FALSE, fig.height=4, fig.width=8}
...
# data:  y ~ x
# GQ = 0.23665, df1 = 98, df2 = 98, p-value = 1
...
```{r b16, warning=FALSE, message=FALSE}
...
# Residuos
		...
		plot(valores.ajustados, residuos, pch=19)
```
...
# Diagnostico de normalidad
		...
		qqline(residuos)
		plot(y,x, xlab = "peso-F", ylab = "peso-b", pch=19)
		abline(y~x)
```
...
library(devtools)  
install_github("dgonxalex80/paqueteDEG", force = TRUE) # bajar paquete
learnr::run_tutorial("Taller_regresion2", "paqueteDEG") # corre instructivo 2
```

<br/><br/><br/>

## **MRL-enfoque matricial** opcional
